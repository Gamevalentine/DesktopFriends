...
| | 时间表系统 (Timemap) | 普通定时器 |
|---|---|---|
| **目的** | Agent 自主管理的语义化日程 | 在固定时间执行预设任务 |
| **调度方式** | HH:mm 时间匹配（Agent 决定内容） | 固定时间或固定间隔 |
| **LLM 消耗** | 仅在条目到期时才调用 LLM | 通常不涉及 LLM |
| **数据来源** | AI 自主创建 + 用户请求 | 开发者或用户预先设置 |
| **持久化** | localStorage（跨会话保留） | 取决于具体实现 |

简单来说：普通定时器只负责"时间到了"，而时间表还会让宠物理解"现在该做什么了？"
