...
跨平台 AI Live2D 桌面宠物应用。支持 **桌面端 (macOS)** 和 **移动端 (Android)**。除了大模型对话与局域网多宠物交流，TableFri 还通过时间表与本地心跳赋予宠物“时间感知”：它不只回答问题，也能安排计划并在合适的时间主动陪伴你。
...
传统 AI 角色通常要等用户先开口。TableFri 的 **时间表（Timemap）系统**允许 Agent 用自然语言任务描述安排当天行为，再由本地心跳在到期时唤醒 Agent：

```text
“下午 3 点提醒我喝水”
        ↓
Agent 创建 15:00 的时间表条目
        ↓
本地心跳每 60 秒检查一次（检查本身不消耗 Token）
        ↓
条目到期 → Agent 结合角色人设执行 → 聊天气泡主动出现
```

它既可响应用户提醒，也能自主安排问候、休息提示和日常互动。一次性任务在当天结束后清理，每日任务则会跨天重置并继续执行。完整说明见 [时间表系统文档](./docs/guide/timemap.md)。
...
### 小组件系统
- **组件即能力** - 启用时钟、天气或待办后，Agent 动态获得对应工具；移除最后一个同类组件时自动撤下
- **桌面独立窗口** - 时钟 / 天气 / 待办使用无背景透明窗口，可单独移动、置顶与关闭
- **移动端网格** - 支持拖拽、缩放、删除和多实例布局
- **共享实时状态** - 天气与待办数据会进入 Agent 上下文；相册组件仍在开发中
...
- **不只是固定定时器** - 时间表是客户端的、Agent 自主管理的语义化日程，由 AI 决定做什么、什么时候做
- **完整管理工具** - Agent 可查看、新增、修改、删除或清空当天计划

> 当前 Android 端已接入完整的心跳触发流程；时间表的数据、工具和调度器位于共享核心包中。应用需要保持运行，Android 后台保活仍在开发中。
...
- **独立对话记录** - 使用普通不透明窗口完整查看、清空并导出 JSON/TXT
- **透明桌面小组件** - 无背景时钟、天气和待办窗口，组件能力与 Agent 动态联动
