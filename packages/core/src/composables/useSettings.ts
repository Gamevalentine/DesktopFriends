...
// 生成唯一 ID
const generateId = () => `pet_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`
...
export const PRESET_BACKGROUNDS = [
  ...
  { id: 'gradient-purple', name: '紫色渐变', value: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  { id: 'gradient-sunset', name: '日落', value: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)' },
  { id: 'gradient-ocean', name: '海洋', value: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
  { id: 'gradient-forest', name: '森林', value: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)' },
  { id: 'gradient-night', name: '夜空', value: 'linear-gradient(135deg, #0c0c0c 0%, #1a1a2e 50%, #16213e 100%)' },
  { id: 'gradient-sakura', name: '樱花', value: 'linear-gradient(135deg, #ffecd2 0%, #fcb69f 100%)' },
  { id: 'gradient-aurora', name: '极光', value: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)' },
  { id: 'gradient-midnight', name: '午夜', value: 'linear-gradient(135deg, #232526 0%, #414345 100%)' },
]
...
export function useSettings() {
  ...
  const currentPet = computed(() => {
    const pet = settings.value.pets.find(p => p.id === settings.value.currentPetId)
  ...
  // 添加新宠物
  const addPet = (config?: Partial<Omit<PetConfig, 'id' | 'createdAt'>>): PetConfig => {
    const newPet: PetConfig = {
      ...
      transformLandscape: config?.transformLandscape || { ...DEFAULT_TRANSFORM },
      createdAt: Date.now(),
    }
  ...
  // 更新宠物配置
  const updatePet = (petId: string, updates: Partial<Omit<PetConfig, 'id' | 'createdAt'>>): boolean => {
    const pet = settings.value.pets.find(p => p.id === petId)
    if (!pet) {
      return false
    }
    ...
    if (updates.transform !== undefined) pet.transform = { ...pet.transform, ...updates.transform }
    if (updates.transformPortrait !== undefined) {
      pet.transformPortrait = { ...(pet.transformPortrait || DEFAULT_TRANSFORM), ...updates.transformPortrait }
    }
    if (updates.transformLandscape !== undefined) {
      pet.transformLandscape = { ...(pet.transformLandscape || DEFAULT_TRANSFORM), ...updates.transformLandscape }
    }
  ...
  const duplicatePet = (petId: string): PetConfig | null => {
    ...
    return addPet({
      ...
      transformPortrait: pet.transformPortrait ? { ...pet.transformPortrait } : undefined,
      transformLandscape: pet.transformLandscape ? { ...pet.transformLandscape } : undefined,
    })
  }
  ...
  const petModel = computed({
    ...
    set: (value: string) => updatePet(currentPet.value.id, { modelPath: value }),
  })
  ...
  // 当前屏幕方向（暴露给外部使用）
  const currentOrientation = computed(() => isLandscape.value ? 'landscape' : 'portrait')

  // 获取当前背景样式
  const backgroundStyle = computed(() => {
    const { backgroundType, backgroundGradient, backgroundImage, backgroundPreset } = settings.value
    ...
    // 默认渐变
    return { background: backgroundGradient || defaultSettings.backgroundGradient }
  })
